...
function rowClass(row) {
  if (row.status === 'denied' || row.status === 'offboarding') return 'is-severe';
  if (row.status === 'pending' || row.status === 'on_leave' || row.status === 'on_hold') return 'is-attention';
  return '';
}
</script>

<template>
  <div class="grove-table-wrap">
    <EmptyState v-if="!rows.length" :title="emptyTitle" :description="emptyDescription" />
    <table v-else class="grove-table">
      <thead>
        <tr>
