...
function rowClass(row) {
  if (row.sla === 'breached' || row.status === 'breached') return 'is-severe';
  if (row.status === 'waiting' || row.status === 'pending' || row.sla === 'at_risk') return 'is-attention';
  return '';
}
</script>
...
<template>
  <div class="lumen-table-wrap">
    ...
    <table v-else class="lumen-table">
      <thead>
        <tr>
