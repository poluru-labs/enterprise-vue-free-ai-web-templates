...
function arcPath(startAngle, endAngle) {
  const start = polar(80, 80, radius, startAngle);
  const end = polar(80, 80, radius, endAngle);
...
<template>
  <div class="vault-donut">
    <svg viewBox="0 0 160 160" role="img" :aria-label="centerLabel">
      <circle cx="80" cy="80" :r="radius" class="vault-donut-track" />
      <path
        ...
        stroke-linecap="butt"
      />
      <text x="80" y="76" text-anchor="middle" class="vault-donut-value">
        {{ centerValue ?? total }}
      </text>
      <text x="80" y="96" text-anchor="middle" class="vault-donut-label">
        {{ centerLabel }}
      </text>
    </svg>
    <ul class="vault-donut-legend">
